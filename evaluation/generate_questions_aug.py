...
eval_questions = ["Is the generated question relevant to the given knowledge?", "Does the generated question **indirectly** refer to the given knowledge?"]
...
# df = pd.read_csv('/sphinx/u/culturebank/tiktok_results/evaluation/cultural_descriptions_v0.csv')
            ...
            try:
                ...
                for i in range(len(eval_questions)):
                    ...
                    eval_messages = [
                        ...
                        {"role": "user", "content": QUESTION_EVAL_USER_TEMPLATE.format(knowledge_line['desc'], json_output["Question"], eval_question)}
                    ]
                ...
                print(f'The generated question does not meet the quality standard, retrying...')
                print(f"Knowledge: {knowledge_line['desc']}")
                print(f"Generated question: {json_output['Question']}")
                print()
                inputs = tokenizer(prompt, return_tensors="pt").to("cuda")
                outputs = text_model.generate(**inputs, max_new_tokens=max_tokens, do_sample=True, temperature=temperature, top_k=top_k, top_p=top_p)
                
                output_text = tokenizer.decode(outputs[0], skip_special_tokens=True)
                output_text = output_text[output_text.rfind('[/INST]')+len('[/INST]'):]
                
                json_output = json.loads(output_text.strip())
                if "Scenario" not in json_output or "Persona" not in json_output or "Question" not in json_output:
                    print(f'returned json object is missing required fields: {json_output}')
                    continue
...
df_results = pd.DataFrame.from_records(df_results, columns=["cluster_id", "scenario", "persona", "question"])
