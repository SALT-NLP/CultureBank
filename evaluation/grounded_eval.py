...
from utils.prompt_utils import (
    ...
    GROUNDED_EVAL_PROMPT_TEMPLATE,
    GROUNDED_EVAL_PROMPT_AUG_TEMPLATE,
)
...
def main():
    ...
    parser.add_argument(
        "--all_questions", action=argparse.BooleanOptionalAction
    )  # whether we benchmark on all questions or just select one question for each knowledge
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df)):
        try:
            ...
            if args.aug:
                ...
                user_message = GROUNDED_EVAL_PROMPT_AUG_TEMPLATE.format(
                    df_line["question"], knowledge_line["desc"]
                )
            else:
